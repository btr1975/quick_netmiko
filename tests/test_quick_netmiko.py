...
from quick_netmiko.quick_netmiko import FailedDnsLookup, BadProtocol, BadSshDeviceType, BadTelnetDeviceType
...
def test_quick_netmiko_bad_device_type_telnet():
    with pytest.raises(BadTelnetDeviceType):
        QuickNetmiko('10.0.0.100', 'unknown', 'temp', 'temp', protocol='telnet')
...
def test_send_single_command_lookup_telnet(monkeypatch, get_ios_test_data_int_brief):
    def mock_init(*args, **kwargs):
        return None
    ...
    nm_obj = QuickNetmiko('FAKE-DEVICE', 'ios', 'temp', 'temp', protocol='telnet')
